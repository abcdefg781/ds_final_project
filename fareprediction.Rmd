...
### Motivation
Here, we want to look at what variables are significant predictors for fare, with the final goal of building a taxi fare estimator that can take values on these selected key variables and produce an estimated fare for users, together with a 95% prediction interval. 


### Exploratory graphs

First, we do some exploratory analysis to provide an brief overview about the outcome variable fare, and how it is distributed across boroughs, neighborhoods, type of taxis, and time of day.
...
Since the data looks heavily right skewed, we decided to drop fares that are above $60, based on our assumption that most of the fares above 60  are mostly negotiated fares. 
```{r include = FALSE, message = FALSE, warning = FALSE}
fare_data = 
...
Next, we fitted the expanded model, as the stepwise regression result suggested.Model diagnostics suggest that observation 123 and 16214 are highly influential points (based on crossing Cook's distance cut-off value), so we removed it. 
...
Fitting the most parsimonious model (with duration and distance as predictors) gives the regression outputs below:
...
And below are the "moderate" model (with duration and distance as predictors) and its regression outputs:
...
This plot above suggests although the moderate model performs only marginal better than the stepwise and parsimonious model, it seems to be the best choice given a balance of both parsimony and better predictive ability. This model also has an R-squared of 88%.
...
(Note that this inference might only be valid for Valentine's Day, R-squared = 88%, and for prices that are less than 60$)
