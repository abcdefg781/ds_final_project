...
d3heatmap(man_man_matrix, scale = "none", colors = viridis(n = 256, option = "viridis"))
```
...
heatmaply(man_man_matrix, colors = viridis(n = 256, option = "magma"), limits = c(1,-1))
```
...
heatmap_function<- function(input_matrix) {
  into_man_heatmap <- d3heatmap(input_matrix, scale = "none", colors = "RdPu") 
...
```{r}
queens_man_matrix <- manhattan_matrix("Queens")
...
```{r}
bronx_man_matrix <- manhattan_matrix("Bronx") 
...
```{r}
staten_island_matrix <- manhattan_matrix("Staten Island")
